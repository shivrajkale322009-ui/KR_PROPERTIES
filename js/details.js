...
async function loadFlatDetails() {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            document.title = `${flat.title} | KR Properties`;

            // Build gallery
            let galleryHTML = flat.imageUrls.map(url =>
            ...
            detailContainer.innerHTML = `
                <div class="detail-grid" style="padding-top: 40px;">
                    <div class="detail-info">
                        <div style="font-size: 2.5rem; font-weight: 800; color: #0b8f5a;">₹${flat.price.toLocaleString()}</div>
                        <h1 style="font-size: 1.8rem; margin-top: 10px;">${flat.title}</h1>
                        <p style="font-size: 1.1rem; color: #4b5563;">${flat.location} | ${flat.size}</p>
                        ...
                        <div style="margin-top: 40px; padding: 25px; background: #f9fafb; border-radius: 16px;">
                            ...
                            <p style="margin-top:15px; font-size: 1.1rem;">${flat.description}</p>
                        </div>
                    </div>
                    ...
                    <div class="detail-sidebar">
                        <div style="background: white; padding: 25px; border-radius: 16px; border: 2px solid #0b8f5a; position: sticky; top: 100px;">
                            ...
                            <a href="https://wa.me/919970434686?text=Interested in ${flat.title}" class="btn-header" style="display: block; text-align: center; background: #25d366;">WhatsApp Now</a>
                        </div>
                    </div>
                </div>
            `;
...
document.addEventListener('DOMContentLoaded', loadFlatDetails);
